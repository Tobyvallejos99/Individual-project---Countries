...
export default function ActivityCreate() {
    ...
    <div className={style.Activity__Container}>
        ...
        <form className={style.Activity__Form} onSubmit={handleSubmit}>
            ...
            <div className={style.Form__Input}>
            <select className={style.Form__Select} onChange={(e) => handleSelect(e)}>
                <option className={style.op}> Countries </option>
                {countries.map((v) => (
                    <option className={style.op} value={v.id}>{v.name}</option>
                ))}
            </select>
            </div>
            ...
            {input.countryId.map((country) => (
                <div>
                <input className={style.Form__Button} type='button' value='X' onClick={() => handleDelete(country)}/>
                <p>{country}</p>
                </div>
            ))}
            </div>
            <div>
            <button className={style.Form__Button} type='submit'>Create Activity</button>
            </div>
        </form>
        </div>
    </div>
